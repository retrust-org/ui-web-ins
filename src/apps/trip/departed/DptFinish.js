...
import ModalFinish from "./DptModalFinish";
...
function DptFinish() {
      ...
      if (extractedId) {
        ...
        switch (extractedId) {
          ...
          case "part6":
            ...
            break;
          default:
            subdomain = process.env.REACT_APP_SUBDOMAIN || "testb2b";
        }
  ...
  return (
    <>
      <section className={styles.FinishWrap}>
        ...
        >
          <div
            ...
            style={{
              backgroundImage: `url('/images/departedBtn.png')`,
            }}
          >
            <button
        ...
        <p className={styles.FinishTextDeparted}>
          잠시만 기다려주세요 <br /> NFT발급에 5초~ 1분까지 걸릴 수 있어요!
        </p>
      </section>

      {/* 파트너 로고 표시 */}
      {process.env.REACT_APP_TYPE === "PARTNER" && partnerId && (
        <div className={styles.floatingContainer}>
          <img
            src={`/images/partner/${partnerId}.png`}
            alt="partner_logo"
            className={styles.floatingImage}
            onError={(e) => {
              e.target.style.display = "none"; // 로드 실패 시 이미지 숨기기
            }}
          />
        </div>
      )}
      ...
      <div>
        <p className={styles.bottomTextDeparted} onClick={openModal}>
          기다리는 동안 알아보기
        </p>
      </div>
      {showModal && (
