...
function GoPay() {
  ...
  const countryData = useSelector((state) => {
    const selected = state.country.selectedCountryData;
    const recommended = state.country.recommendedCountryData;
    return selected || recommended || {};
  });

  const cityNatlCd = "KR00";
  const engCityNm = countryData.engCityNm || "";
  const korNatlNm = countryData.korNatlNm || "";
  ...
  const insuranceData = {
    ...
    natlCd: "KR00",
    inspeCnt: contractInfos.length,
    contractInfos: contractInfos,
  };

  const mintData = {
    productCd: priceData?.productCd,
    card_quantity: contractInfos.length,
  ...
  const nicepayStart = () => {
    ...
    payForm.method = "POST";
    payForm.action = `${process.env.REACT_APP_BASE_URL}/api/v3/pay/authReq`;
    ...
    payForm.appendChild(
      createInputElement(
        ...
        process.env.NODE_ENV === "development"
          ? `${process.env.REACT_APP_REDIRECT_URL}/api/v3/pay/authReq`
          : `${window.location.origin}/api/v3/pay/authReq`
      )
    );
