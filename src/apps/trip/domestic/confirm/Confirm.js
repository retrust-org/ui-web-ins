...
import { calculateInsuranceAge, calculateManAge, formatKoreanDate, getTodayYYYYMMDD } from "../../../../utils/birthDate";
...
function Confirm() {
  ...
  // 앱 타입에 따라 PLAN_LIST 필터링
  const filteredPlanList = useMemo(() => {
    // DOMESTIC 앱에서는 추천 플랜 제외, 다른 앱에서는 ACTIVITY 플랜 제외
    return PLAN_LIST.filter(
      (plan) =>
        plan.dataKey !== "RECOMMEND" && // 추천 플랜 항상 제외
  ...
  useEffect(() => {
    if (activePlan === "ACTIVITY" && appType !== "DOMESTIC") {
      setActivePlan("BASIC");
      dispatch(setSelectedPlanName(PLAN_TYPES.BASIC));
      dispatch(
        selectData({ name: PLAN_TYPES.BASIC, price: priceData?.BASIC?.ttPrem })
      );
    }
  ...
  useEffect(() => {
    if (userInfo.dateOfBirth) {
      ...
      // 보험나이 계산
      const insuranceAge = calculateInsuranceAge(userInfo.dateOfBirth, baseDate);
  ...
  const renderInsuredPersons = () => (
    <div className={styles.dataFlexbox}>
      ...
      <div className="flex flex-col">
        <span>
          {getGenderString(userInfo.gender)} (만 {userAge}세 / 보험나이 {userInsuranceAge}세)
          ...
          <div className={styles.filterBtnWrap}>
            <ul>
              {filteredPlanList.map((plan, index) => (
                <li
                  key={index}
                  className={`cursor-pointer w-full mx-auto ${plan.dataKey === activePlan ? styles.active : ""
                    }`}
