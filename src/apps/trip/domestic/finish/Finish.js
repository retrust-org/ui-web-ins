...
function Finish() {
  ...
  const { nation, city, start_date, end_date } =
  ...
  // URL에서 파트너 ID 추출 및 매핑
  useEffect(() => {
    // 현재 앱 타입이 PARTNER가 아니면 처리하지 않음
    if (process.env.REACT_APP_TYPE !== "PARTNER") {
      return;
    }
  ...
  return (
    <>
      ...
      {process.env.REACT_APP_TYPE === "PARTNER" && partnerId && !imageError && (
