...
function Member() {
  ...
  // window 객체에서 직접 reOrderData 접근
  const reOrderUserData = window.reOrderData?.user || {};
  ...
  // window.reOrderData 데이터로 필드 초기화 (컴포넌트 마운트 시 한 번)
  useEffect(() => {
    const windowReOrderData = window.reOrderData?.user;
    if (windowReOrderData) {
      if (windowReOrderData.name && !name) setName(windowReOrderData.name);
      if (windowReOrderData.name_english && !englishName)
        setEnglishName(windowReOrderData.name_english.toUpperCase());
      if (windowReOrderData.phone && !phoneNumber)
        setPhoneNumber(windowReOrderData.phone);
      if (windowReOrderData.email && !email) setEmail(windowReOrderData.email);
    }
  }, []);
