...
import { getGenderString } from "./Gender";
...
function Confirm() {
  ...
  const [userAge, setUserAge] = useState(""); // 만 나이
  const [userInsuranceAge, setUserInsuranceAge] = useState(""); // 보험나이 추가
  ...
  // 필터링된 플랜 목록 - 'ACTIVITY' 제외
  const filteredPlanList = useMemo(() => {
    return PLAN_LIST.filter((plan) => plan.dataKey !== "ACTIVITY");
  }, []);
  ...
  // ACTIVITY 플랜이 선택되어 있는 경우 기본 플랜으로 전환
  useEffect(() => {
    if (activePlan === "ACTIVITY") {
      setActivePlan("BASIC");
  ...
  useEffect(() => {
    if (selectedData) {
      const planType = PLAN_KEYS[selectedData.name];
      if (planType && priceData?.[planType] && planType !== "ACTIVITY") {
