...
function SignupChkConsent() {
  ...
  const { contractorType, updateAuthData } = useDisasterHouse();
  ...
  const handleAuthSuccess = async (result) => {
    ...
    // Context에서 contractorType 확인하여 분기 처리
    if (contractorType === '01') {
      // 개인: 바로 userInfo로 이동
      navigate('/userInfo');
    } else {
      // 사업자: BusinessInfo 페이지로 이동
      navigate('/userInfo/business');
    }
  };
  ...
  return (
    <>
      <DisasterHeader title="가입 전 확인사항" backPath="/personalInfoConsent" />
